...
the top of each file. **Do not type the list from memory, and do not use the
`LIVING_DOCS` constant — it is deliberately narrower than the estate** and feeds
only the countable guards. Enumerate the docs with
`doc_sync_check.living_stamped_docs`, which takes every tracked `.md` at its own
word; the freshness guard judges all of those, so stamping a subset leaves the
rest to go red.
