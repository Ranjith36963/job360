...
authority. The Loop-3 PR-time gate (`scripts/doc_sync_check.py`, run by
`.github/workflows/doc-sync.yml` on every pull request) also discovers every doc
that STAMPS itself LIVING, reads both the type tag and the date, and flags any
doc not verified within 45 days — so stamping a hardcoded subset leaves the rest
to go red on freshness at the next PR.
