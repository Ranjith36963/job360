...
- Auth round-trip: POST /api/auth/register (new account) **then always** POST /api/auth/login,
  ...
  - `/register` deliberately does **not** log you in — `backend/src/api/routes/auth.py`
    ...
    (`backend/src/api/routes/auth.py`). Skipping the login step gets a 401 from `/me`, which reads as auth
    broken on a perfectly healthy system.
  - The route is `/api/auth/me` — `backend/src/api/routes/auth.py` `APIRouter(prefix="/auth")`
    + `backend/src/api/routes/auth.py` `@router.get("/me")`, mounted under `/api`. There is no bare `/api/me`; it 404s.
