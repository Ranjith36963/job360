...
description: >-
  ...
  sources, scoring, auth routes, notifications, or profile extraction.
---
<!-- doc: LIVING | last-verified: 2026-08-25 by the CLAUDE.md diet -->
...
21. **Value-presence > schema-presence for new engine-side fields.** `assert "field" in body` passes against a `= 0` default and a serializer that never reads the column. Run a real input end-to-end and assert non-default. Pattern: `backend/tests/test_database.py::test_dim_columns_round_trip`.
