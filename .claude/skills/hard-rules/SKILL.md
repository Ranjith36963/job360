...
10. **Never INSERT into `jobs` with `user_id`/`tenant_id`** — `jobs` is the shared catalog (a brought job is a catalog row too). Per-user state lives in the application spine's tables (`applications`, `application_events`, `application_artifacts`, `application_receipts`).
