...
8 + 13. **Adding/removing a source = FIVE surfaces:** `SOURCE_REGISTRY`, `_build_sources()`, `RATE_LIMITS`, and the hardcoded registry counts in `backend/tests/test_cli.py` and `backend/tests/test_api.py` — all five move together.
...
20. **Multi-dim scoring is gated on `user_preferences` ALONE** — a missing `enrichment_lookup` gives each dim its documented NEUTRAL half, never zeros (#29). Guard: `backend/tests/test_scorer.py::TestScoreBreakdown::test_dims_neutral_not_zero_when_enrichment_missing`.
