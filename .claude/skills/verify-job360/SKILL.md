...
description: >-
  ...
  touch backend (FastAPI, routes, the application spine, DB) or frontend (Next.js pages,
  ...
  queries the Postgres DB for backend, and walks the full register→CV→bring→receipt
...
- **Backend** — you changed a route, the spine, the DB or a service → run it, hit the route, query the DB, read the logs.
...
- **Did the row land?** Query Postgres directly (SQLite was fully removed — the
  ...
  (never print the DSN).
  Useful tables: `jobs` (shared catalog), `user_profiles`, `users`, `sessions`,
  `applications`, `application_events`, `application_artifacts`, `application_receipts`.
...
3. **Bring a job** on `/bring` (paste an ad, or a link — `POST /jobs/fetch-url`) or `POST /api/jobs/bring`.
...
and the DB/log evidence), severity, and the fix. Report the `CHECKLIST.md` PASS/FAIL/GATED
table alongside it.
