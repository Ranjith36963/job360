...
- **The search routes are OFF by default** — `POST /api/search` and `GET /api/search/{run_id}/status` answer **404**, and `/dashboard` + bare `/jobs` 404 in `frontend/src/middleware.ts`. Not a bug and not an auth failure: `backend/tests/test_search_flag.py::test_search_routes_404_when_the_flag_is_off` pins it, and `::test_search_routes_work_when_on` shows what turning it on takes.
