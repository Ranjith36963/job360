...
def _send_sync(msg: MIMEMultipart):
    try:
        ...
        raise RuntimeError("SMTP authentication failed — check SMTP_EMAIL and SMTP_PASSWORD") from None
...
async def send_email(jobs: list[Job], stats: dict, csv_path: str | None = None):
    ...
    logger.info("Email sent to %s with %s jobs", NOTIFY_EMAIL, len(jobs))
