...
- **Canonical pre-commit verification:** `cd backend && python -m pytest -q -p no:randomly` (1,528 passing, 3 skipped on Windows; 1,531 collected — defer to the runtime collected count). `test_main.py` is included: it was rehabbed offline in the M8 batch (JobSpy stubbed via autouse fixture) and runs in ~8 s.
...
**1,528 tests passing, 3 skipped** (1,531 collected — defer to the runtime collected count), 0 failing (bash-only `setup.sh` / `cron_run.sh` tests skip on Windows). Shared fixtures in `tests/conftest.py`. All HTTP mocked with `aioresponses`. `pytest-asyncio` for async tests.
