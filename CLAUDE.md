...
28. **STRICT — ZERO hardcoded skill/keyword lists in profile extraction (`src/services/profile/`).** Extraction is ESCO ontology (`data/esco/`) + LLM only. **Banned:** any `*_SKILL_TERMS` / `*_TO_SKILL` / skill-keyword dict or denylist — hand-typed maps overfit one CV and inflate eval scores dishonestly. The deterministic pass does STRUCTURE only (sections/bullets/exact tokens) + ESCO vocabulary; prose→skill mapping belongs to the LLM. **Carve-out: `core/skill_synonyms.py` is RETAINED** — it is scoring/search vocabulary (`skill_matcher.py`, `keyword_generator.py`), reads no CV/LinkedIn/GitHub input, and must not be deleted under this rule.
