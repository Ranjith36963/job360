...
- **Canonical pre-commit verification:** `cd backend && python -m pytest -q -p no:randomly`. **Never quote a test count from a doc — measure it** (`python -m pytest --collect-only -q | tail -1`); three docs once disagreed by 400–800 tests. The suite runs against a **real Postgres** (docker-compose.dev.yml, host port 5433) via the `sqlite3`/`aiosqlite` shims in `tests/conftest.py`, schema-per-test — not SQLite. HTTP is mocked with `aioresponses`; the suite must run offline. `test_main.py` is part of the canonical run (JobSpy stubbed offline in M8, ~8 s) — do **not** re-add `--ignore=tests/test_main.py`.
...
- **What automation is actually running:** the **GitHub Actions harness** — 22 workflows in `.github/workflows/` (repair, triage, doc-sync, ci, ci-offline, codeql, security, uptime, live-e2e, journey, product-health, db-backup, pr-shepherd, dependabot-auto…). The old agent loop (worker/integrator/scout/health, `docs/maintenance/MISSIONS.md`) is **DORMANT — disabled 2026-06-21**; those SKILL.md files carry DORMANT banners. **Do not wait on it.**
- **What surprises new sessions:** `SOURCE_REGISTRY` has 47 entries but 46 unique classes (`indeed` + `glassdoor` both alias `JobSpySource`) — measure with `python -c "from src.main import SOURCE_REGISTRY as R; print(len(R), len(set(R.values())))"`. Heavy deps must be lazy-imported (#11/#16). Next.js 16 made `params` async (#22). Adding a source touches **five** files (#8/#13). Migrations auto-apply on FastAPI boot via `lifespan` in `src/api/dependencies.py`.
...
30. **UK-only is a DOOR, not a penalty; never hand-enumerate an UNBOUNDED set.** Foreign jobs are refused at ONE chokepoint (`services/uk_gate.check_uk`, called at `main.py:1040`) — never per-source, never a scorer penalty. Foreign cities are unbounded so they are never typed; UK places are finite, so the gate matches DATA (`data/uk_gazetteer/`, built by `scripts/build_uk_gazetteer.py`). Countries/admin divisions stay enumerated only because those are CLOSED sets. The country override must run BEFORE gazetteer matching. **Dry-run any location rule over the live catalog first** — the naive version blocked 48%. Guard: `tests/test_uk_gate.py`.
...
### Scoring + enrichment
29. **"Filled shelves work harder; empty shelves stay SILENT."** An empty preference (salary, locations, workplace, experience, about_me) means "don't care" — never a penalty, never a per-job zero, never a guess. Dim scorers return a CONSTANT for an empty user side; prefilter stages pass everything; the judge prompt omits unset prefs; the frontend never blocks on one. Guard: `tests/test_design_rules.py`.
9. **Scoring changes require running `test_scorer.py` AND `test_profile.py`.**
18. **Pillar 2 flags default off.** With `ENRICHMENT_ENABLED`/`SEMANTIC_ENABLED` false, behaviour must *exactly* match pre-Pillar-2 — no implicit semantic queries, no LLM calls. Same rule applies to `MATCHER_ENABLED` (engine #4).
19. **`JobScorer` legacy default = 4-component formula** (Title 40 / Skill 40 / Location 10 / Recency 10). 7-dim scoring activates only when all three kwargs are passed. Don't flip defaults silently.
...
28. **STRICT — ZERO hardcoded skill/keyword lists in profile extraction (`src/services/profile/`).** *(Standing owner preference — still binds.)* **Banned:** any `*_SKILL_TERMS` / `*_TO_SKILL` / skill-keyword dict or denylist — hand-typed maps overfit one CV and inflate eval scores dishonestly; prose→skill mapping belongs to the LLM. *(Fact, verified 2026-08-11.)* Extraction today is **LLM + a purely STRUCTURAL deterministic pass** (headings/sections/bullets — `_det_collect_section`, `cv_parser.py:469`); **no ontology is consulted.** **ESCO is inert scaffolding, not live behaviour** — `data/esco/{labels.json,embeddings.npy}` is gitignored (`.gitignore:24` `data/`, with exemptions for `uk_gazetteer`/`job_signals` only), stripped from the image (`backend/.dockerignore:2`), generated by no CI/Docker/setup step, absent from every disk, and **was never built at all** (`docs/_archive/pillar2_implementation_plan.md:54`); its single consumer `_maybe_normalise_skills_via_esco` (`cv_parser.py:796`) is an identity no-op behind `SEMANTIC_ENABLED` (default **false**, `settings.py:214`) **and** `is_available()`, the loader degrades silently on missing artefacts (`skill_normalizer.py:93-95`, `logger.debug` only), so `ProfileResponse.skill_esco` (`api/models.py:238`) is **always `{}` in prod**. Never cite ESCO as running; reviving it means shipping the artefacts (build script + `.gitignore`/`.dockerignore` exemptions), not flipping the flag. **Carve-out: `core/skill_synonyms.py` is RETAINED** — it is scoring/search vocabulary (`skill_matcher.py`, `keyword_generator.py`), reads no CV/LinkedIn/GitHub input, and must not be deleted under this rule.
...
24. **`notify_mode` = `instant` | `daily` | `every_n_hours`.** `instant` sends inline; the others (and anything caught in quiet hours) queue into `user_notification_digests`, drained by the `notification_tick` ARQ cron → `send_bundle` (dispatches with `force=True`, marks `sent` / retries / `dlq` after 5). The only delivery path is worker/tick → `dispatcher.dispatch()` → Apprise → `notification_ledger`. New dispatch paths need tests for all three modes AND both quiet-hours states.
