...
15. **New sources MUST set `.category`** to one of: `"ats"`, `"rss"`, `"keyed_api"`, `"free_json"`, `"scrapers"`, `"other"` — or add a `NAME_TIER[source.name]` override in `scheduler.py`. Untagged sources fall to the 60-min default. (The scraper tier key in `scheduler.py` is `"scrapers"` — plural; the 5 HTML scrapers set `category = "scrapers"` to match, fixed in `33768b9`.)
...
28. **STRICT — ZERO hardcoded skill/keyword lists in profile extraction (`src/services/profile/`).** Extraction is **data-driven (ESCO ontology, loaded from `data/esco/`) + LLM** only. Hand-typed skill maps overfit one CV, are brittle (new skill = code edit), and inflate eval scores dishonestly. **Banned in code:** any `*_SKILL_TERMS` / `*_TO_SKILL` / skill-keyword dict/denylist. Offenders being removed: `cv_parser._PROSE_SKILL_TERMS` / `_COMMON_TOOL_TERMS`, `github_enricher._DESC_SKILL_TERMS` / `LANGUAGE_TO_SKILL` / `TOPIC_TO_SKILL` / `_DEV_TOOLING_DENYLIST`, `dependency_map.py`. **Sanctioned exception (kept on purpose):** `core/skill_synonyms.py` — a curated canonical-alias dictionary (spelling/variant → canonical form, e.g. `js`→`javascript`, `k8s`→`kubernetes`). It only *normalizes* known skill spellings at **scoring / keyword-canonicalization** time (`skill_matcher.py`, `services/profile/keyword_generator.py`); it does **not** infer skills from prose. This is the one allowed hardcoded map — offline, O(1), zero API cost — so it stays; everything else in this rule stays banned. Deterministic pass = STRUCTURE only (sections/bullets/exact tokens) + ESCO-vocabulary match; semantic prose→skill mapping belongs to the LLM (prompt-steering is fine, that's not hardcoding). If you find a *skill-inference* keyword list in extraction (not `skill_synonyms`), remove it and route through ESCO data or the LLM.
