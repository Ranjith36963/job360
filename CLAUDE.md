...
- **Canonical pre-commit verification:** `cd backend && python -m pytest -q -p no:randomly`. **Never quote a test count from a doc — measure it** (`python -m pytest --collect-only -q | tail -1`); three docs once disagreed by 400–800 tests. Runs against a **real Postgres** (docker-compose.dev.yml, port 5433), one schema per test via `src.repositories.pg.TEST_MODE` / `pg.schema_for_path`. HTTP is mocked with `aioresponses`; the suite must run offline. Never add `--ignore=` for a test file; fix or delete the test.
...
- What automation is actually running: the GitHub Actions harness in `.github/workflows/` (count: `docs/GENERATED.md`; triage, doc-sync, ci, ci-offline, codeql, security, uptime, db-backup, pr-shepherd, branch-reaper…; measure with `ls`). The old agent loop (scout/worker/integrator) was disabled 2026-06-21 and its files deleted in the cleanup audit (#503) — there is no background agent to wait on.
...
**Product path:** `POST /jobs/bring` (`api/routes/bring.py`) stores the ad as a `jobs` row and births one Application (`services/applications/spine.py`: `applications` + append-only `application_events` / `application_artifacts` / `application_receipts`) → tailoring as web fallback (`api/routes/tailor.py`) → MCP server `api/mcp_server.py` (tool count: measure with `grep -cF '@mcp.tool()' backend/src/api/mcp_server.py`; bearer tokens `j360_…` or OAuth 2.1; `/api/mcp`). Nothing scores, ranks, dedups or enriches a job — that pipeline was deleted in slice 5 (#483).

- `src/repositories/pg.py` is the single DB door — an `aiosqlite`-shaped async driver whose `translate()` rewrites legacy SQLite SQL to Postgres at runtime. It is production-critical, not test-only (guard: `tests/test_pg_translate.py`).
