...
> fetch on the web — **built on branch `feat/url-fetch` (stacked on slice 2), awaiting
> review/PR**: `POST /jobs/fetch-url` fills `/bring` from a link (JSON-LD → meta →
> heuristic ladder) under a new `src/services/fetch/` SSRF guard drilled ten ways
> (`scripts/ssrf_drill.py`); paste stays the fallback, no MCP tool → 4 contacts/outreach,
> stats, `update_profile`. Measure: the owner uses it daily for his own hunt.
