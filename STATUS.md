...
Four engines are available, stacked **keyword → dimensions → hybrid → LLM judge**. All default OFF except the keyword engine. The table of services, flags and defaults lives once, in `ARCHITECTURE.md` § "Matching Engine Stack"; the gates themselves are pinned by `backend/tests/test_engine_flag_gates.py` and the defaults by `backend/tests/test_engine_switches.py::test_engine_switch_defaults`.

Measured on Engine #4: 18/18 judged in 89.8 s at concurrency 3; judge spread 20–92 vs keyword 30–43; 10/10 fit accuracy on labeled sample.
