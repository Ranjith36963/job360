...
Branch: `worktree-channels-notifications-overhaul`. Spec: `docs/harness/superpowers/specs/2026-06-17-channels-notifications-overhaul-design.md`; plan: `docs/harness/superpowers/plans/2026-06-17-channels-notifications-overhaul.md`.
...
Branch: `step-3-batch` off `main @ 9868877` (Step 2 green tip). Plan: `docs/harness/step_3_plan.md` (preserved on commit `df36c8f`).
...
`docs/harness/step_3_plan.md` (preserved on commit `df36c8f`; not currently on `main` because of the planned-but-never-run relocation that gave `5d47d59` its "pre-relocation snapshot" message).
...
**Reference:** `docs/product/research/pillar_3_batch_1.md` · Plan: `docs/product/plans/batch-1-plan.md`
...
9. **docs/product/plans/batch-1-plan.md** — the TDD plan this batch followed, with clean-main baseline locked at top.
...
- `docs/product/plans/batch-1-plan.md` — new (the TDD plan).
- `docs/harness/IMPLEMENTATION_LOG.md` — this completion entry.
...
**Status:** MERGED to main 2026-04-18 via `6446feb` (`--no-ff` merge of `pillar3/batch-2` @ `d124ed5`) after 3 reviewer rounds (report: `docs/harness/reviews/batch-2-review.md`, final verdict APPROVED at `f5c3395` on `pillar3/batch-2-review`).

**Reference:** `docs/product/research/pillar_3_batch_2.md` · Plan: `docs/product/plans/batch-2-plan.md` · Decisions: `docs/product/plans/batch-2-decisions.md`
...
- `(this commit)` `docs: P1-1 overclaim reword + D6 reconciliation + TODO markers` — the reword in §"What shipped" item 3 above, TODO markers above `insert_action` + `create_application` in `backend/src/repositories/database.py`, and the D6 REVISION block in `docs/product/plans/batch-2-decisions.md`.
...
- `docs/product/plans/batch-2-decisions.md` — new (brainstorming output)
- `docs/product/plans/batch-2-plan.md` — new (TDD plan with locked baseline)
- `docs/harness/IMPLEMENTATION_LOG.md` — this completion entry
...
**Reference:** `docs/product/research/pillar_3_batch_3.md` · Plan: `docs/product/plans/batch-3-plan.md`
...
8. **`docs/product/plans/batch-3-plan.md`** — the TDD plan this batch followed (378 lines), with POST-BATCH-2 baseline locked at the top.
...
- `docs/product/plans/batch-3-plan.md` — new (the TDD plan).
- `docs/harness/IMPLEMENTATION_LOG.md` — this completion entry.
...
**Reference:** `docs/product/research/pillar_3_batch_4.md`
...
**Reference:** `docs/product/plans/batch-3.5-plan.md`
...
### KPI deltas (where measurable)
  ...
  - **Multi-user safety:** Before — two real users hitting
    ...
    and clobber each other (docs/harness/IMPLEMENTATION_LOG.md Batch 2 entry §P1-1
...
- `docs/product/plans/batch-3.5-plan.md` — new (the TDD plan; 150 lines).
- `docs/harness/IMPLEMENTATION_LOG.md` — this completion entry.
...
- `harness/eval/pytest_baseline_seeds.txt` — 11 deterministic seeds for reproducible runs.
...
- `mypy strict=true` with heavy-dep overrides; `docs/harness/mypy_backlog.md` captures 395 suppressible warnings (documented, not suppressed — future-pass backlog).
...
- **Step 1 (Batch S1 — Engine→API seam):** date-model fields, 7-dim score breakdown, `enrich_job()` invocation, `mode=hybrid` wiring. See `docs/harness/ExecutionOrder.md` for the full 6-step roadmap.
...
### Phase summaries (current invariants only — full history in `docs/harness/IMPLEMENTATION_LOG.md`)

The codebase has accumulated 4 major phases of work. Each section here gives **only the invariants that future Claude needs to respect**. Long-form history (what shipped when, reviewer findings, deferred items, decision rationales) lives in `docs/harness/IMPLEMENTATION_LOG.md` and the per-step plan files under `docs/`.
