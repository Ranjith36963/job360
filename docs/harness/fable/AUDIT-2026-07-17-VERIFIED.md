...
- **Area:** Harness & Workflow (docs/harness/fable/06-HARNESS-AND-WORKFLOW.md + fable-harness-plan.md + FABLE_FINDINGS.md H7)
...
- **Area:** Production Signals (docs/harness/fable/09-PRODUCTION-SIGNALS.md)
...
- **Area:** FRONTEND & AUTH-UI
- **Claimed:** FIXED (903af8d) per docs/harness/fable/03-FRONTEND-AND-AUTH-UI.md:30
...
- **Area:** FRONTEND & AUTH-UI
- **Claimed:** FIXED per docs/harness/fable/03-FRONTEND-AND-AUTH-UI.md:30 (bundled with the dashboard error-state fix)
...
- **Area:** FRONTEND & AUTH-UI
- **Claimed:** docs/harness/fable/03-FRONTEND-AND-AUTH-UI.md P1 says FIXED (1c75b50) but only names 2 of the 4 pages (reset-password, verify-email); FABLE_FINDINGS.md H11 lists all 4 (reset-password, verify-email, forgot-password, auth/magic)
...
- **Area:** FRONTEND & AUTH-UI
- **Claimed:** FIXED (b939e29) per docs/harness/fable/03-FRONTEND-AND-AUTH-UI.md:30 — fail-closed, redirects to /login?error=service_unavailable WITHOUT deleting the cookie
...
- **Proof (current code):** backend/src/sources/base.py:32-52 — `_is_uk_or_remote` now uses `re.search(rf"\b{re.escape(term)}\b", loc_lower)` for UK_TERMS, REMOTE_TERMS, and FOREIGN_INDICATORS, with a docstring explicitly stating '(S3 fix; see docs/harness/FABLE_FINDINGS.md)' and calling out Milwaukee/Ukraine as the exact false-positive cases the finding named.
...
- **Proof (current code):** CLAUDE.md:347 '### Tailored documents — AI CV & cover-letter generator (2026-07-04)' — full phase-summary paragraph describing services/tailoring/*, routes/tailor.py, migrations 0023/0024, frontend components, TAILOR_FREE_PER_MONTH, and links docs/product/peruser_cv_coverletter.md.
