...
|---|---|---|---|
| B1 | **Plan doc** | `pr:` header **written by CI, never hand-typed** · PR merged · **not reverted** (no `Revert "…"` of its merge SHA on `origin/main`) · its diff touches/mentions the doc | add at the TOP of the file, verbatim, `> **IMPLEMENTED** in PR #N (`<sha>`) — archived <date>` (§2 of DOC-MAINTENANCE.md) plus `<!-- doc: FROZEN -->`, `git mv` → `docs/_archive/`, then grep the repo for the old path and repair EVERY inbound link |
