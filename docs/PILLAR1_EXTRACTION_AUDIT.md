...
| 6 | The consumer is double-gated | `_maybe_normalise_skills_via_esco` runs only when `SEMANTIC_ENABLED` **and** `is_available()`; `SEMANTIC_ENABLED` defaults **false** (`backend/src/config/settings.py:214`) |
...
**Reviving ESCO is a shipping problem, not a flag flip.** In order: build the artefacts, add
