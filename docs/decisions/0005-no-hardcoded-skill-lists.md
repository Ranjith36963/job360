...
2026-08-25 from hard rule #28 and a 2026-08-11 code audit (see "Still valid?" below)
...
extraction path: ESCO's artefacts are gitignored, stripped from the Docker
image, generated by no build step, and absent from every disk; its consumer
degrades silently when they are missing. `.claude/skills/hard-rules/SKILL.md`
rule #28 carries the full chain.
