...
9. ✅ **DONE (33768b9) — matcher telemetry, incl. persistence:** `MatcherTelemetry` (`backend/src/utils/telemetry.py:81-107`) tracks judged/skipped/failed/fit-spread, and `llm_matcher.py:200-234` calls `matcher_telemetry().record_verdict(...)` per verdict. **Persistence now landed:** `MatcherTelemetry.as_dict()` (`telemetry.py:109`) is snapshotted into the new `run_log.matcher_stats` TEXT column (`database.py:133`, auto-adds on boot) via `log_run(matcher_stats=...)` (`database.py:437`), with a per-run reset. Round-trip verified: `run_log.matcher_stats = {"judged":10,"avg_fit":45.5,"fit_min":12,"fit_max":90}`. "Morning review without grepping logs" is now satisfied.
