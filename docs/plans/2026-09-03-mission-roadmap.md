...
| 9 | **Flag for next time** — a Flag box on the receipt page writes a `lesson` event; a Lessons list on the profile; `get_profile` returns the last N lessons | 2 | the owner flags one lesson, sees it on the profile, and Claude Code gets it back from `get_profile` | #516 |
