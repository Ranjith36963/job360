...
| 5 | **Delete the sourcing era** — search routes, `SOURCE_REGISTRY` + 41 sources, batch scorer / judge / enrichment, feed tables no longer read, the sourcing-only workflows and their drills; `add-source` skill and pillars 02/03 moved to `docs/_archive/` | 2 live for one release | `grep -r SOURCE_REGISTRY` finds nothing; test count drops and CI is green; ARCHITECTURE.md describes only what remains | #483 — built on `feat/delete-sourcing-era`, draft PR (owner merges after #488/#496/#498) |
