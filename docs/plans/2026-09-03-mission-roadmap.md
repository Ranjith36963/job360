...
| 4 | **Contacts, outreach, stats, profile edits** — `add_contact`, `outreach` artifact kind, `stats` (reply / interview rate per CV version, per role), `update_profile` | 2 | the agent adds a recruiter and an outreach message to an application; `stats` returns counts that match a hand count of the event log; a profile field edited by the agent shows on the web | #482 — shipped, PR #498 |
| 5 | **Delete the sourcing era** — search routes, `SOURCE_REGISTRY` + 41 sources, batch scorer / judge / enrichment, feed tables no longer read, the sourcing-only workflows and their drills; `add-source` skill and pillars 02/03 moved to `docs/_archive/` | 2 live for one release | `grep -r SOURCE_REGISTRY` finds nothing; test count drops and CI is green; ARCHITECTURE.md describes only what remains | #483 — shipped, PR #499 (2026-09-05); cleanup audit #503 followed |
| 6 | **Email evidence on events + interview datetime** — the agent's own Gmail connector reads the inbox and calls us; we add an optional `source` (`{kind: email, message_id, from, subject, received_at}`) on `record_event`, idempotent per `(application, message_id)`, and a real `scheduled_at` on `interview_*` events. No Gmail OAuth, no polling from us. | 2 | in a real browser, Claude.ai + Gmail connector logs a rejection, an invite with a date and a plain reply; the same read twice adds zero rows; the timeline shows sender/subject | #513 |
| 7 | **Visa / sponsorship signal, country-agnostic** — `visa_signal` / `visa_detail` / `visa_country` (ISO alpha-2) on `bring_job` / `save_fit` and a web dropdown (agent judges, we store); the profile's `work_authorization_countries` list; `needs_sponsorship` = list membership, `null` when either side is silent; badge on card + detail. `jobs.visa_flag` left for its own cleanup (8 source + 4 test files name it) | 2 | `bring_job(visa_signal="no_sponsorship", visa_country="DE")` from Claude Code shows the red badge on the web; `unknown` shows nothing; a country in the candidate's list shows the muted "not needed for you" | #514 — shipped 2026-09-11, `docs/plans/2026-09-11-visa-signal/spec.md` |
| 8 | **CV diff, read-only** — original vs any version side by side on the application page, changed lines marked on both panes; the version the receipt names carries an Applied badge; **no Keep button** (VISION decision 26 — the receipt, not a button, says which version counts); provenance colouring stays | 2 | Playwright: open an application with two cv versions and a receipt naming v2, see the Applied badge on v2, press Compare, see a removed line marked left and an added line marked right, find no Keep | #515 — `docs/plans/2026-09-11-cv-diff/` |
| 9 | **Flag for next time** — a Flag box under the application timeline writes a `lesson` event; a Lessons list on the profile (`GET /applications/lessons`, newest first, superseded excluded); `get_profile` returns the last `PROFILE_LESSONS_MAX` lessons | 2 | the owner flags one lesson, sees it on the profile, and Claude Code gets it back from `get_profile` | #516 — shipped 2026-09-11, `docs/plans/2026-09-11-lessons/spec.md` |
| 10 | **Later, on evidence only** — WhatsApp (needs worker + Redis back; inbound-only Twilio webhook is the cheap door if ever), multiple named profiles, recruiter side (consent-first) | the owner using it daily | not scheduled | — |

Slices 6–9 come from the 2026-09-07 competitor read,
[`docs/product/competitor-tsenta.md`](../product/competitor-tsenta.md).
...
issues for them. The competitor table says the same thing row by row.
