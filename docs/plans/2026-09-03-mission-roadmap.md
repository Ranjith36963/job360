...
| 7 | **Visa / sponsorship signal** — `visa_signal` input on `bring_job` / `save_fit` (agent judges, we store), badge on the card next to the profile's `cv_right_to_work`; dead `jobs.visa_flag` folded in or dropped | 2 | `bring_job(visa_signal="no_sponsorship")` from Claude Code shows the badge on the web; `unknown` shows nothing | #514 |
