...
CV headings/sections/bullets (`_det_collect_section`, `backend/src/services/profile/cv_parser.py:469`),
...
(`github_enricher.py:640,678`). None of these is an ontology lookup.
...
| 6 | The consumer is double-gated | `_maybe_normalise_skills_via_esco` runs only when `SEMANTIC_ENABLED` **and** `is_available()`; `SEMANTIC_ENABLED` defaults **false** (`backend/src/core/settings.py:214`) |
...
| 8 | So the output is always empty | `ProfileResponse.skill_esco` (`backend/src/api/models.py:238`) is permanently `{}` in production |
...
```
cv_parser.py:878    ← the CV parsing path
schemas.py:215      ← the LLM-schema adapter path (imports it at schemas.py:28)
```
