...
3. **Rotate the rest of the keys** (names only, values live in Railway):
   ...
   Each provider's dashboard → revoke old, issue new. Rotate every one of them
   freely — nothing stores per-user third-party credentials any more, so no
   rotation here costs a user anything.
...
| `applications`, `application_events`, `application_artifacts`, `application_receipts` | job-hunt activity (who applied where, and what was sent) | Medium — sensitive in context (current employer must not learn) |
...
whichever comes first. Last verified against the real stack: 2026-09-08.*
