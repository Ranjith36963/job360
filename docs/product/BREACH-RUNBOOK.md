...
| the application-spine tables (`applications` + `application_events` / `_artifacts` / `_receipts` / `_contacts`) | job-hunt activity (who applied where, and the evidence) | Medium — sensitive in context (current employer must not learn) |
| `sessions`, `oauth_states` | session + OAuth artifacts | Low once rotated/deleted |
| `jobs` | job ads users brought | Not personal data |
