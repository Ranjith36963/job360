...
2. **Verifies** it by restoring into a throwaway Postgres. Only two gates are enforced: `users >= 1` and `count(_schema_migrations) >= 20`. **`jobs` is printed, never asserted** — an empty catalog does NOT fail the job.
