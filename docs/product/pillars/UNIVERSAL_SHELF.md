...
| Free derivation | `services/salary.normalize_salary` annualises via `_FREQUENCY_ANNUAL` and converts via `core/fx.to_gbp` (18 hardcoded rates, 2026-Q1, `core/fx._RATES_TO_GBP`); `core/fx.is_known_currency` guards display in `api/routes/jobs`; remotive's regex parser (correctly skips hourly); nhs regex (broken: splits on '.', decimal hourly rates never parse — fix in gate) |
...
**Model:** the enrichment chain is OpenAI-primary — `llm_extract_validated` → `gpt-4o-mini` (paid, deterministic), then free-tier Gemini `gemini-3.7-flash` → Groq → Cerebras fallbacks (the provider order lives in `services/profile/llm_provider.llm_extract`). Cost math below prices the primary; fallbacks only fire on OpenAI failure.
