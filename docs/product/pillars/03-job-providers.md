...
> **All sources skip gracefully without their key**: the keyed-source pattern is `if not api_key: return []` with a **`WARNING`** log line — Adzuna, Careerjet, Findwork, GoogleJobs, Jooble, JSearch and Reed all use `logger.warning`; `gov_apprenticeships.py:62` is the lone `INFO`. A missing key never fails the run — that source returns `[]` and simply doesn't contribute, and **the run still reports success**, so the log line is the only place it shows. (That is a statement about missing CREDENTIALS, not a claim that the pipeline cannot error for other reasons.)
