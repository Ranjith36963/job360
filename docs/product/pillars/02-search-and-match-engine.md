...
- Gate: **a budget, not a threshold**, in `run_search`'s enrich stage. Jobs must clear a low floor — `match_score >= ENRICHMENT_MIN_SCORE` (default **10**) — and then the best `ENRICHMENT_MAX_JOBS` (default **20**) are sent to the LLM. `ENRICHMENT_THRESHOLD` is not read at this call site — it defaults to `ENRICHMENT_MIN_SCORE` when unset. It is **not** dead weight for old `.env` files, though: the ARQ worker's per-job enqueue path in `workers.tasks.score_and_ingest` is still gated on it, so setting it high stops worker-side fan-out while leaving this CLI selection untouched — see the env table at §5. Why the change: the scorer's contract is 0–100, but the highest `match_score` measured across the whole 3,342-row prod feed on 2026-07-28 was **58**, so a threshold of 60 selected nothing and the stage had never run. A budget makes no claim about the distribution, so it cannot go stale.
...
Either name opens this surface at every E2 call site (rule #18). Enumerating those sites here only rots; `backend/tests/test_engine_flag_gates.py` fails if one ever reads the legacy name alone.
...
- ESCO skill normalisation in CV parsing does **not** flip on. `_maybe_normalise_skills_via_esco()` is double-gated: `SEMANTIC_ENABLED` **and** `skill_normalizer.is_available()`, and the index artefacts have never been built, so it stays an identity transform either way (rule #28; `docs/product/PILLAR1_EXTRACTION_AUDIT.md`).
...
| `SEMANTIC_ENABLED` | `false` | Writes embeddings into the pgvector store — the write path reads this name ALONE, so `ENGINE3_ENABLED` by itself queries an index nothing fills (pinned by `backend/tests/test_engine_flag_gates.py`). It does **not** switch ESCO on: that also needs `skill_normalizer.is_available()` and the index artefacts have never been built | Flip on after `pip install ".[semantic]"`; ~300 MB of deps |
