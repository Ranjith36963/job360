...
```sql
-- or straight from psql. Schema: `migrations/runner._ensure_table`.
...
The optional second argument is a `db_path`, **not** a migration stem — it goes to
`repositories/pg.connect`, and migrations always run against `DATABASE_URL`. Pinned by
`tests/test_migrations.py::test_down_reverses_last_migration`.
