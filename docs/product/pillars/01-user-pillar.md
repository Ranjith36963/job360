...
| `APP_ENV` / `RAILWAY_ENVIRONMENT` | no (Railway sets the latter) | (unset) | The real production switch. Either one makes `_is_production()` true → `Secure` session cookie + HSTS, and makes `validate_required_env()` refuse to boot when `SESSION_SECRET`, `CHANNEL_ENCRYPTION_KEY` or `DATABASE_URL` is empty (`core/settings.py:384-400`). |
...
| --- | --- | --- | --- |
| API refuses to start: `RuntimeError` naming `SESSION_SECRET` / `CHANNEL_ENCRYPTION_KEY` / `DATABASE_URL` | Env var missing **in production** | FastAPI `lifespan` → `validate_required_env()` (`api/main.py:114`, `core/settings.py:387-400`) | Set the env var; fail-closed by design |
| Dev/CLI instead: server starts fine, then a 500 on the first request that needs a session or a channel credential | Same vars missing, but `validate_required_env()` is a **no-op outside prod** and the CLI never builds the FastAPI app, so nothing boot-checks them | `auth_deps._secret()` (`:36-41`) / `channels/crypto._fernet()` (`:19-33`) raise at CALL time | Set the env var. Do not read the boot row above as "it always fails at startup" — outside prod it fails later, further from the cause |
