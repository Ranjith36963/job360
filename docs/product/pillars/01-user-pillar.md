...
Click "Apply" → browser opens external `apply_url`. Returning, Alice clicks "Mark Applied". Frontend `POST /api/pipeline/42` — the job id is in the PATH, there is no `/applications` collection route and no request body (`frontend/src/lib/api.ts`, `backend/src/api/routes/pipeline.py`). Route (`routes/pipeline.py`):

1. **404** if the job id is unknown; **410 Gone** if `jobs.staleness_state='confirmed_expired'` (ghost-detection guard, `routes/pipeline.py`).
2. `INSERT OR IGNORE INTO applications(user_id, job_id=42, stage='applied')` — and that is the whole write (`repositories/database.py`).

Two things this step does **not** do, both worth knowing before you debug the pipeline:

- **No `application_stage_history` row is written on create.** The only INSERT into that table lives in `advance_application` (`repositories/database.py`), so a just-created application has an empty history and `GET /api/pipeline/42/timeline` answers **404** until Alice moves it to a second stage (`routes/pipeline.py`).
- **`user_feed.status` is not flipped to `'applied'`.** The column exists and its comment lists `'applied'` as a legal value (`migrations/0003_user_feed.up.sql`), but no production path ever puts that value there. Be precise about why, because the obvious grep misleads twice. `FeedService.update_status` (`services/feed.py`) *would* write any status string, and it has **no caller under `backend/src/`** — only `tests/test_feed_service.py`. Every other write to the column is a hard-coded `'active'` or `'stale'` (`feed.py`, `rescore.py`). And the `'applied'` literals that *do* exist belong to two other columns entirely: `user_actions.action` (`routes/actions.py`, read at `rescore.py`) and `applications.stage` (`database.py`; `routes/pipeline.py` validates stage names). The card's applied state is read from `applications`, not from the feed row.

### T+30 — Alice sets up email notifications

On the channels page:

- `POST /api/settings/channels {channel_type:"email", display_name:"primary", credential:"alice@example.com"}` — for `email` the `credential` is a **plain address**, not an Apprise URL. Anything that is not a valid address is refused 422 (`backend/src/api/routes/channels.py`).
- The backend builds the Apprise URL itself from the PLATFORM's mail credentials — `build_email_apprise_url(dest)` (`services/channels/email_url.py`) — then `channels.crypto.encrypt(...)` → Fernet ciphertext → INSERT `user_channels`. With no mail transport configured the route returns 503 "email delivery is not configured" (`channels.py`).
- `slack`, `discord` and `telegram` cannot be created here at all: they are Connect-flow only and this route returns 400 for them (`channels.py`).
...
After an interview: `POST /api/pipeline/42/advance {"stage": "interview"}`. The body model is one field — `PipelineAdvanceRequest.stage` (`backend/src/api/models.py`); there is no `to_stage` key and the endpoint accepts no `notes`. Route validates against `_VALID_STAGES` (`routes/pipeline.py`) then calls `advance_application`, which updates `applications.stage='interview'`, sets `last_advanced_at=now()`, and INSERTs an `application_stage_history` row with `from_stage='applied'` — the two writes share one transaction (`repositories/database.py`). It does **not** touch `notes_history`: that column is written only by `PATCH /api/pipeline/{job_id}/notes` → `update_application_notes` (`database.py`).
...
- `frontend/src/components/layout/Navbar.tsx` — top-of-page nav. Shows user email and a Log out button when authenticated; shows Log in / Register otherwise. The session cookie is `HttpOnly`, so the frontend never *reads* it directly — it just relies on the browser to send it and checks the `/api/auth/me` response.
...
### 2.5 OAuth 2.1 for MCP clients (migration 0036, 2026-09-03)

Job360 is its own OAuth 2.1 **authorization server** so ChatGPT connectors and Claude.ai integrations can connect to `https://job360.uk/api/mcp` through the browser login (`docs/plans/2026-09-03-oauth-mcp/spec.md`). Personal `j360_…` tokens stay as the CLI fallback.

- **Discovery** — `GET /.well-known/oauth-authorization-server` (RFC 8414) and `/.well-known/oauth-protected-resource[/api/mcp]` (RFC 9728) are mounted at the site **root** (`routes/well_known.py`; three `next.config.ts` rewrites carry them to the backend). A 401 from `/api/mcp` carries `WWW-Authenticate: Bearer resource_metadata="…"` so a client finds the server on its own.
- **Flow** — `POST /api/oauth/register` (RFC 7591, public clients only, redirect URIs must pass `OAUTH_REDIRECT_ALLOWLIST`) → `GET /api/oauth/authorize` (PKCE S256 required; client + exact redirect URI verified **before** any redirect) → 302 to the consent page `/oauth/consent/[rid]` (session only; a logged-out user is bounced to `/login?next=…` and the magic-link email now carries `next`) → `POST /api/oauth/authorize/{rid}/decision` → `POST /api/oauth/token` (form-encoded; atomic `UPDATE … RETURNING` claim; code reuse after `OAUTH_REUSE_GRACE_SECONDS` revokes the grant; refresh rotation; absolute 30-day refresh expiry).
- **Tokens** — access `j360a_…` (1 h) and refresh `j360r_…`; only SHA-256 hashes are stored; every access token carries an `audience` that `/api/mcp` checks. `auth_deps.py` dispatches on the prefix: `j360a_` → `CurrentUser(auth_via="oauth")`, which behaves like a personal token everywhere except the session-only credential routes (`require_session_user` → 403).
- **Revocation** — `POST /api/oauth/revoke` (RFC 7009, always 200) and the **Connected apps** list on `/settings/connect` (`GET /api/oauth/grants` + `DELETE /api/oauth/grants/{grant_id}`, session only) both revoke the whole grant; the next request under it fails (the grant row is read live, no cache).
- **Tables** — `oauth_clients`, `oauth_authorization_requests`, `oauth_grants` (one active per user+client, partial unique index), `oauth_authorization_codes`, `oauth_tokens`. `oauth_grants` is a per-user table (erased with the account, included in the export). `pg.py` strips FK clauses, so `database.py` deletes the child rows explicitly.
- **Limits** — per-IP and global registration caps, an authorize cap, a `/token` failure throttle, a separate OAuth bearer-failure throttle (only a no-match counts), a client cap with prune-at-ceiling, and sampled housekeeping after `/token`. All tunables are `OAUTH_*` settings (table below).
...
- `GET /api/profile/json-resume` — exports the profile in [JSON Resume](https://jsonresume.org/) schema. The path is **hyphenated** (`routes/profile.py`); `jsonresume` is not a route
...
- `send_bundle(ctx, user_id)` (`tasks.py`) — drains queued rows from `user_notification_digests` across **all** the user's channels, batches into one Apprise call per channel, marks rows `sent`. There is no `send_daily_digest`; the per-channel task of that name was removed with the one-rule-per-user collapse.
- `notification_tick(ctx)` (`tasks.py`) — the 5-minute cron that decides which users are due and enqueues `send_bundle`.
...
>
> **No ESCO badges on `/profile`.** The page does carry an "ESCO mappings" block (`profile/page.tsx`), but it renders only when `profile.skill_esco` is non-empty, and that map is filled only when `SEMANTIC_ENABLED` is on **and** the ESCO index artefacts are present on disk (`backend/src/services/profile/cv_parser.py` returns the identity transform otherwise). Hard rule #28: those artefacts are inert scaffolding, never built and never shipped. So in production the block never draws — do not describe it as something a user sees.
...
> **The 8D breakdown on `JobResponse` is not the list this doc used to print.** The eight dimensions the engine actually computes are `role`, `skill`, `location_score`, `recency`, `seniority_score`, `salary_score`, `visa_score`, `workplace_score` (`backend/src/api/models.py`). `experience`, `credentials` and `semantic` are also on the model, but they are **legacy dead columns from migration `0011` that the engine never produced** (`models.py`) — a UI that draws them draws zeros forever. `penalty` is not a dimension either. Never sum the eight to get the total: the raw max is 130 clamped to `[0, 100]` (rule #27) and the −30 negative-title penalty lands on no dimension, so `match_score` is the only truth for the total.
...
| --- | --- | --- | --- |
| `SESSION_SECRET` | **yes, everywhere** | **none — there is no dev fallback** | HMAC secret for session cookies. Rotating invalidates every active session. Fail-closed at CALL time: `auth_deps._secret()` raises `RuntimeError` whenever it is unset, in dev as well as prod (`backend/src/api/auth_deps.py`). `settings.py` deliberately does **not** bind it as a module constant — the old `os.getenv(..., "")` default read as "an empty secret is a valid state" and was removed in M10 (`core/settings.py`). Also boot-checked in prod, below. |
| `CHANNEL_ENCRYPTION_KEY` | **yes, everywhere** | **none — there is no dev fallback** | Fernet key for `user_channels.credential_encrypted`. Same fail-closed shape: `channels/crypto.py` raises if unset. **Do not rotate without a re-encryption migration** — existing channels become undecryptable. |
| ~~`JOB360_ENV`~~ | — | — | **DEAD — read by nothing under `backend/src/`.** The only trace is a comment at `api/routes/auth.py`. The session cookie's `Secure` flag now gates on the same signal as HSTS, Sentry and CORS: `middleware._is_production()`, i.e. `APP_ENV=production` **or** `RAILWAY_ENVIRONMENT` set (`api/middleware.py`, used by `_set_session_cookie` at `auth.py`). Setting `JOB360_ENV=prod` on its own changes nothing, and on Railway it was never set at all. |
...
| `OPENAI_API_KEY` (+ `OPENAI_MODEL`, default `gpt-4o-mini`) | no | (unset) | **First-choice** LLM for CV parsing — the chain is OpenAI → Gemini → Groq → Cerebras (`services/profile/llm_provider.py`). A lowercase `openai_api_key` is accepted too (`core/settings.py`). |
...
| `CEREBRAS_API_KEY` | no | (unset) | Last-choice LLM. **All FOUR unset** → `require_llm_key()` raises `LLMKeyMissing` before any provider is tried (`llm_provider.py`); it subclasses `LLMError(RuntimeError)`, which is what the 502 in the failure table below is made of. |
...
| `OAUTH_REDIRECT_ALLOWLIST` | no | Claude.ai + ChatGPT callbacks | Comma-separated redirect URIs a registering OAuth client may use — exact scheme/host/port, path equal or (entry ending in `/`) prefix. Anything else → 400 `invalid_redirect_uri`. Add a new MCP client here, never in code (`services/auth/oauth_clients.py`) |
| `OAUTH_ALLOW_LOOPBACK_REDIRECTS` | no | `0` | `1` also allows RFC 8252 loopback redirects (`http://127.0.0.1`, `::1`, `localhost`, any port) for desktop clients. Off in prod |
| `OAUTH_ACCESS_TOKEN_TTL_SECONDS` / `OAUTH_REFRESH_TOKEN_TTL_SECONDS` | no | `3600` / `2592000` | Access-token life and the **absolute** refresh life of a grant (counted from the grant, not the last refresh). After the second, the user consents again |
| `OAUTH_CODE_TTL_SECONDS` / `OAUTH_AUTHORIZE_TTL_SECONDS` | no | `60` / `1800` | Authorization-code life, and how long a pending consent request (`/oauth/consent/[rid]`) survives — long enough for the magic-link email round-trip |
| `OAUTH_REUSE_GRACE_SECONDS` | no | `10` | A code or refresh token presented twice inside this window is a client retry → `invalid_grant`; after it → the whole grant is revoked (OAuth 2.1 §4.1.2) |
| `OAUTH_REGISTER_MAX_PER_HOUR` / `OAUTH_REGISTER_MAX_PER_HOUR_GLOBAL` | no | `60` / `600` | Dynamic-registration caps per IP and across all IPs (429 over cap) |
| `OAUTH_AUTHORIZE_MAX_PER_MIN` | no | `60` | Per-IP cap on `GET /api/oauth/authorize` |
| `OAUTH_TOKEN_FAIL_MAX_PER_MIN` | no | `30` | Failed `/api/oauth/token` exchanges per IP (and per IP+client) per minute before 429 |
| `OAUTH_BEARER_FAIL_MAX_PER_MIN` | no | `30` | Failed `Authorization: Bearer j360a_…` attempts per IP per minute — its own bucket, only a hash matching **no** row counts (an expired token is normal, not an attack) |
| `OAUTH_CLIENT_PRUNE_DAYS` / `OAUTH_MAX_CLIENTS` / `OAUTH_PRUNE_SAMPLE` | no | `7` / `10000` / `20` | Housekeeping: a registered client with no grant ever is dropped after N days; the client table is capped (prune-at-ceiling, then 503); the prune runs after 1-in-N `/token` calls, in its own transaction, never delaying the exchange |
| `NOTIFY_EMAIL` | no | — | **DEAD at application runtime.** Declared at `core/settings.py` and read by nothing under `backend/src/`, `backend/tests/` or `scripts/`. It belonged to the pre-Batch-2 single-tenant notifier that was deleted, so setting it sends no user a thing. The name survives outside the app: `cron_setup.sh` greps `.env` for it to print a "configured" line, and `setup.sh`/`setup.bat` mention it. |
| ~~`SLACK_WEBHOOK_URL` / `DISCORD_WEBHOOK_URL` / `TELEGRAM_*`~~ | **REMOVED** | — | Deleted from settings on 2026-08-24 with the chat channels themselves (`core/settings.py` documents the removal). **Do not confuse with the unrelated, still-live `SLACK_WEBHOOK_URL` repo secret** that `.github/actions/slack` uses to page the owner when a build breaks — the harness talking to the owner, not the product talking to a user. Untouched. |
...
| Not this row: `DATABASE_URL` | It has a working dev default (`postgresql://job360:job360dev@localhost:5433/job360`, `core/settings.py`), so outside the gate it is never *missing* — it is merely pointed somewhere | A bad or unreachable DSN surfaces as a connection error from `pg`/`pgsync`, never from `_secret()` or `_fernet()` — those two only ever cover the session and channel secrets | Check the DSN and that Postgres is up (`docker-compose.dev.yml`, port 5433) |
...
| CV upload returns 502; profile fields empty | All **four** LLM providers (OpenAI → Gemini → Groq → Cerebras) exhausted / returned malformed JSON twice | `parse_cv_async` raises — `LLMKeyMissing` if no key is set at all, else `LLMAllProvidersFailed` / `LLMRateLimited`; all three subclass `LLMError(RuntimeError)` (`llm_provider.py`), and the route surfaces 502 | Check provider env vars; tail logs filtered by `cv_parser`; try a smaller/cleaner PDF |
...
| LLM provider fallback (OpenAI → Gemini → Groq → Cerebras) | ✅ | `llm_provider.py` |
