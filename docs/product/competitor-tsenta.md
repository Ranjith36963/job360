...
<!-- doc: LIVING | decided 2026-09-07 by the owner; status column measured against main 6e30fb1 -->
...
| # | What Tsenta does | Our call | Why | State on main (2026-09-07) |
|---|---|---|---|---|
...
| 5 | Tailored CV + cover letter per job, "review before submit" with a diff | **Copy** | We tailor already. Add the **side-by-side diff + Keep button**. | Tailor done (`backend/src/api/routes/tailor.py`). Diff **not built** — only provenance colouring. → #515 |
| 6 | Sponsorship signals surfaced | **Copy** (was wrongly "have") | `jobs.visa_flag` is dead code since slice 5; nothing writes it, nothing shows it. Only the CV-side `cv_right_to_work` lives. Agent supplies the signal, we store + show. | **Not built** → #514 |
...
| 8 | "Flag for next time" on a receipt | **Copy** | The learning loop. Cheap. | Backend done (`lesson` event). **No web button, no lessons list** → #516 |
| 9 | Gmail connected: reads replies / invites / rejections, updates the tracker | **Copy, agent-side** | The agentic part. The user's own agent reads Gmail through its connector and calls our store tools. **No Gmail OAuth from us** (VISION decision 8). | Store tools exist; email evidence + interview datetime **missing** → #513 |
...
| 12 | Interview dates + recruiter messages shown | **Copy** | Falls out of #9. | Events `interview_requested/scheduled/done` + contacts done; **date is free text** → #513 |
...
| 18 | MCP for Claude Code / Claude / Cursor / Codex — find roles, tailor, submit | **Copy, different tools** | Ours store, theirs do. | **Done** — 18 tools; `bring_job`, `save_fit`, `tailor_documents`, `record_application`, `get_receipt`, `record_event` cover the six we named. ChatGPT + Grok connectors: to be proved live in prod |
