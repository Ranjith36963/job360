...
| 18 | MCP for Claude Code / Claude / Cursor / Codex — find roles, tailor, submit | **Copy, different tools** | Ours store, theirs do. | **Done** — 18 tools; `bring_job`, `save_fit`, `tailor_documents`, `record_application`, `get_receipt`, `record_event` cover the six we named. **Claude.ai / ChatGPT / Grok / Cursor connectors still NOT proved live** (2026-09-11: prod has 0 OAuth grants; each client's callback URL must be in `OAUTH_REDIRECT_ALLOWLIST` on Railway — Cursor's included — and a logged-in browser must do the connect once; VISION decision 24, the one open row of the feature audit. No client of any kind has connected to prod: 0 OAuth grants AND 0 personal `j360_…` tokens ever minted, measured 2026-09-11. The MCP surface is exercised only by the test suite, `backend/tests/test_mcp_server.py`.) |
