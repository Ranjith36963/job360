...
- **Semantic (engine #3)** — `services/embeddings.py` (all-MiniLM-L6-v2, 384-dim, chunked) + `vector_index.py` (ChromaDB at backend/data/chroma) + `retrieval.py` (`retrieve_for_user`: RRF k=60 fusing keyword `match_score` + **BM25 (`bm25_rank`)** + vector ANN, then **cross-encoder rerank** (`cross_encoder_rerank`, ms-marco-MiniLM); wired live via `_hybrid_reorder_rows`); surfaced via `GET /api/jobs?mode=hybrid` (`_maybe_apply_hybrid_reorder`, profile-query text).
...
- **Known issues:** judge telemetry surface added 2026-06-16 (`MatcherTelemetry` in `utils/telemetry.py` — judged/skipped/failed + fit min/avg/max; populated by `match_batch`; per-run fit-spread logged by `_run_matcher_stage`), but **not yet persisted into `run_log`** (#9 partial); the run pipeline loads `load_profile(user_id)` per-user on the HTTP path, but the CLI path requires `data/user_profile.json` (web-vs-CLI profile split — verify-skill gotcha). (#8 re-judge-on-profile-change: DONE 2026-06-13.)
