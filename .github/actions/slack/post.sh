...
#   SLACK_API_BASE    TEST SEAM ONLY. Default https://slack.com/api. See the
#                     allow-list below — it exists so backend/tests/
#                     test_slack_voice.py can drive this exact file against a
#                     localhost stub, offline, instead of re-implementing it in
#                     Python and testing the copy. A guard tested through a
#                     re-implementation is testing the re-implementation.
#
# Exit codes:
...
# ── THE ENDPOINT, AND WHY IT IS ALLOW-LISTED ───────────────────────────────
# SLACK_API_BASE exists so the tests can run this file offline. But this script
# sends a live bot token as a bearer credential, so a freely-overridable URL
# would be a token-exfiltration path: anything that can set one env var could
# redirect the Authorization header to a server it controls. So the override is
# allow-listed to loopback. Not a theoretical risk — workflow env is exactly the
# surface a malicious PR tries to influence.
API_BASE="${SLACK_API_BASE:-https://slack.com/api}"
case "$API_BASE" in
  https://slack.com/api) ;;
  http://127.0.0.1:*|http://localhost:*) ;;
  *) fail "SLACK_API_BASE='$API_BASE' is not allowed. It may only be the real Slack API or a loopback test stub — refusing to send the bot token to an arbitrary host." ;;
esac

resp_file="$(mktemp)"
# ── THE PAYLOAD GOES IN ON STDIN, NOT IN ARGV ──────────────────────────────
# `-d "$payload"` looks identical and silently corrupts non-ASCII. MEASURED:
# the payload leaving jq held a correct UTF-8 em-dash (bytes e2 80 94); the
# bytes curl actually PUT ON THE WIRE were 0x97 — the CP1252 em-dash, which is
# not valid UTF-8 at all. Git Bash converts argv from UTF-8 to the machine's
# ANSI codepage when handing off to a native .exe, so the corruption happens
# after every check in this script has already passed. Emoji became `??`.
#
# That is Windows-only (ubuntu runners have no such conversion), but stdin is
# the better call on every platform anyway: it also sidesteps the argv length
# limit, and these bodies carry scraped log text and model output that can get
# long. `--data-binary` because `-d` mangles newlines.
#
# It matters here specifically because this repo writes em-dashes everywhere —
# a mangled alert body is a corrupted alert, and there is already a guard in
# this repo (scripts/encoding_guard.py) for the same class of bug.
code=$(printf '%s' "$payload" | curl -sS --max-time 25 -o "$resp_file" -w '%{http_code}' \
  -X POST "$API_BASE/chat.postMessage" \
  -H "Authorization: Bearer $TOKEN" \
  -H "Content-Type: application/json; charset=utf-8" \
  --data-binary @-) || fail "curl could not reach $API_BASE/chat.postMessage (network/transport error)."
