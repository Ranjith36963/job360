...
# Register it with cron — see docs/harness/maintenance/HEALTH-SCHEDULE.md.
#
...
DAILY="$REPO/docs/harness/maintenance/STATUS-DAILY.md"
...
claude -p "Run the /health skill: do the daily three-pillar system check and overwrite docs/harness/maintenance/STATUS-DAILY.md with the report. Collect verbatim evidence. Mark any check that cannot run as UNKNOWN with the reason."
