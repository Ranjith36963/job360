...
def test_sentry_scrub_removes_sensitive_headers_and_password():
    from src.core.observability import _scrub_pii as _scrub_event
...
def test_sentry_init_disables_default_pii(monkeypatch):
    ...
    The init now lives in ``src.core.observability`` (shared by the API and the
    ARQ worker — docs/fable/09 P0); the API's ``_init_sentry`` delegates to it.
    """
    import sentry_sdk
    ...
    # init_sentry only reports from a deployed prod env (prod-gate). Mark prod so
    # it reaches the init() call whose PII settings this test asserts on.
    monkeypatch.setenv("RAILWAY_ENVIRONMENT", "production")

    assert observability.init_sentry(component="api") is True
